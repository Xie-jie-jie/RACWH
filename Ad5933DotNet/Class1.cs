...
namespace Ad5933DotNet
{
    ...
    public class UsbDevice
    {
        ...
        /// <param name="ms">最长等待时间(ms)</param>
        /// <returns>如果成功，返回实际读取长度，否则返回值小于0</returns> 
    ...
    public class AD5933
    {
        ...
        /// <param name="ms">最长等待时间(ms)</param>
        /// <returns>true 成功 false 失败</returns>
        public bool Read(uint ms)
        {
            int err = usb.Read(rxBuff, 20, ms);
            if (err < 0) return false;
            if (rxBuff[0] != 65 || rxBuff[17] != 68) return false;
            Info.Freq = (uint)((rxBuff[4] & 0xFF) | ((rxBuff[3] & 0xFF) << 8) | ((rxBuff[2] & 0xFF) << 16) | ((rxBuff[1] & 0xFF) << 24));
            Info.Real = (int)((rxBuff[8] & 0xFF) | ((rxBuff[7] & 0xFF) << 8) | ((rxBuff[6] & 0xFF) << 16) | ((rxBuff[5] & 0xFF) << 24));
            Info.Image = (int)((rxBuff[12] & 0xFF) | ((rxBuff[11] & 0xFF) << 8) | ((rxBuff[10] & 0xFF) << 16) | ((rxBuff[9] & 0xFF) << 24));
            Info.Temp = (ushort)((rxBuff[14] & 0xFF) | ((rxBuff[13] & 0xFF) << 8));
